...
$(function(){
	...
	addValidator('#receiveBankName',bankNameRule);
	var validator = $('#trade-form').kendoValidator({
		rules:{
            ...
            receiveAccountNoRule : function(input) {
            	if(input[0]==$('#receiveAccountNo')[0]){
            		...
            		if(tradeType == 'WITHDRAW_FUND'){
            			if(accountNo.length > 19 || accountNo.length < 16){
                			input.attr('data-receiveAccountNoRule-msg','账号必须为16-19位');
                			return false;
                		}
            ...
            receiveAccountNameRule : function(input){
            	if(input[0]==$('#receiveAccountName')[0] && tradeType == 'TRANSFER_FUND'){
	...
	function checkTransferFund(input) {
		...
		if(tradeType == "WITHDRAW_FUND"){
			commissionCharge = calculateCommissionCharge(amount);
		...
		if(totalAmount> balance){
			input.attr("data-transferFundRule-msg", "余额不足");
			return false;
		}
		return true;
	 ...
	 * @returns {String}
	 */
	function calculateCommissionCharge(amount){
		return cana.bank.withdrawFee(amount);
	}
