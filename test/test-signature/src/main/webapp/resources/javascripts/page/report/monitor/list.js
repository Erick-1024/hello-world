$(function() {
	...
	$("body").on("click", ".detail-link", function(e){
		...
		window.open(encodeURI(basepath + "/report/monitor/detail?memberId=" + memberId + "&outCustomerId=" + outCustomerId + "&customerName=" + customerName + "&usedLimit=" + usedLimit + "&pledgeRage=" + pledgeRage));
	});
	...
	$("#monitorList-grid").kendoGrid({
		...
		columns : [{
		            ...
		            template: function(data){
		            	var url = "";
		            	if(detailAuth)
		            		url += "<a class='comRow-link detail-link' href='javascript:void(0);' data-memberId="+ data.memberId +" data-outCustomerId="+ data.outCustomerId+" data-customerName="+data.customerName+" data-usedLimit="+data.usedLimit+" data-pledgeRage="+data.pledgeRage+">详情</a>";
		            	return url;
		            },
					editable : "popup"
				}],
