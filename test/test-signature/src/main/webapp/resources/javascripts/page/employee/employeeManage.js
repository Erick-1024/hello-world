		...
		$.ajax({
			...
			success: function(data){
				if(data.status == "SUCCESS"){
						...
						$("span[name='roleNameForShow']").text(data.data.roleName == null ? "":data.data.roleName);
		...
		$.ajax({
			...
			success: function(data){
				if(data.status == "SUCCESS"){
					...
					$("#roleForEdit").data("kendoDropDownList").value(data.data.roleId);
	...
	$("body").delegate("#saveBtnForAdd", "click", function(e) {
		...
		if(!validator.validate()) {
 		...
 		$.ajax({
 			...
 			data: {id:$.trim($("span[name='idForAdd']").text()),realName:$.trim($("input[name='realNameForAdd']").val()),employeeMail:$.trim($("input[name='contactMailForAdd']").val()),jobNo:$.trim($("input[name='jobNoForAdd']").val()),
 				roleId:$.trim($("#roleForAdd").val()),employeeTel:$.trim($("input[name='contactTelForAdd']").val()),employeeJobTitle:$.trim($("input[name='jobTitleForAdd']").val())},
 				dataType: "json",
 				success: function(data){
 					if(data.status == "SUCCESS"){
 						var win = $(e.target).closest(".k-window");
...
function editInputCheck(){
	...
	$("body").delegate("#saveBtnForEdit", "click", function(e) {
		validator = $("#editEmployeeForm").kendoValidator().data("kendoValidator");
		if(!validator.validate()) {
    		return;
 		};
		$.ajax({
            ...
            data: {id:$.trim($("input[name='idForUpdate']").val()),employeeMail:$.trim($("input[name='contactMailForEdit']").val()),jobNo:$.trim($("input[name='jobNoForEdit']").val()),
            		roleId:$.trim($("#roleForEdit").val()),employeeTel:$.trim($("input[name='contactTelForEdit']").val()),employeeJobTitle:$.trim($("input[name='jobTitleForEdit']").val())},
            dataType: "json",
            success: function(data){
            	if(data){
                    var win = $(e.target).closest(".k-window");
