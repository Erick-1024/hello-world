	...
	$("#repayManage-grid").kendoGrid({
		...
		dataSource:{
	        ...
	        },
	        {
                ...
                {
                	var link = "";
                	if(offlineRepaymentAuth){
                		if(data.accessToAdjustAndOfflineRepayment){
                			link += "<a target='_blank' class='comRow-link incRole-link' href='"+basepath+"/repayment/adjustment/toAdjustment?loanInfoId=" + data.id + "'>还款</a>";
                		}
                	}
                	return link;
                }
                ...
                template: function(data)
                {
                	if(setAccountNoAuth)
                	{
		            	if(null == data.accountNo || "" == data.accountNo)
		            	{
		            		return "<a id='sethello' class='comRow-link setAccount' href='javascript:void(0);' data-factorcompany=" + data.factorCompany + " data-financecompany=" +data.financeCompany + " data-loanno=" +data.loanNo + " data-loanid=" +data.id +">设置</a>"
		            	}
		            	else
		            		return "";
                	}
