function modifyRule() {
	...
	var validator = $("#ruleEdit-form").kendoValidator({
		rules : {
	...
	$("body").on("click", "#modifyRule-button", function() {
		if(!validator.validate()) {
			return;
		};
		var data;
		if("DEFAULT" == repaymentRuleType){
			data = {
					id: $.trim($("#ruleId").text()),
					factorTransferInAccountNo: $.trim($("#factorTransferInAccountNo").val().parseBankAccountNo()),
			...
			data = {
					...
					fianceCustomerIds: ids,
					fianceCustomerCompanys: companyNames,
					deductionTime: $.trim($("#deductionTime").val()),
					deductionRule: $.trim($("#deductionRule .active").data("deductionrule")),
					extensionDays: $.trim($("#extensionDays").val()),
					extensionRatio: $.trim($("#extensionRatio").val()),
		...
		$.post(
			basepath + "/repayment/rule/modifyRule",
			data ,
			function(data) {
