 ...
 * 
 */
public class TestRSA2048P1 extends TestFrame {
    ...
    final String signedMessage(byte[] message) throws UnsupportedEncodingException, PKIException {
        return new String(new Signature().p1SignMessage(Mechanism.SHA256_RSA, message, priKey, session));
    }
    ...
    final boolean verifiedMessage(byte[] message, byte[] signature) throws UnsupportedEncodingException, PKIException {
        return new Signature().p1VerifyMessage(Mechanism.SHA256_RSA, message, signature, cert.getPublicKey(), session);
    }
    ...
    public static void main(String[] args) throws Exception {
        TestFrame.runTest(new TestRSA2048P1(), true, args);
    }
