 ...
 * 
 */
public class TestRSA2048P7Attach extends TestFrame {
    ...
    final String signedMessage(byte[] message) throws UnsupportedEncodingException, PKIException {
        return new String(new Signature().p7SignMessageAttach(Mechanism.SHA256_RSA, message, priKey, cert, session));
    }
    ...
    final boolean verifiedMessage(byte[] message, byte[] signature) throws UnsupportedEncodingException, PKIException {
        return new Signature().p7VerifyMessageAttach(signature, session);
    }
    ...
    public static void main(String[] args) throws Exception {
        TestFrame.runTest(new TestRSA2048P7Attach(), true, args);
    }
