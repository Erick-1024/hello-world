...
public class CreditLimitEffectRetryTaskHandler extends AbstractRetryTaskHandler {
...
private Logger logger = LoggerFactory.getLogger(getClass());
	...
	private String notifyType = "额度生效通知";
	...
	public void execute(RetryTask task, HandlerStatus status) throws Exception {
		...
		try {
			NotifyTzResultRetryTaskData<AsyncNotifyTzCustomerCreditLimitResult> data = gson.fromJson(task.getData(), new TypeToken<NotifyTzResultRetryTaskData<AsyncNotifyTzCustomerCreditLimitResult>>(){}.getType());
			if(!"SUCCESS".equals(travelzenDataApiImpl.sendCreditLimitFeedback(data.getData(), data.getUrl()).getRetCode())) {
				status.fail();
