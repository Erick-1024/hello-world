...
public class RepaymentSuccessNotifyRetryTaskHandler extends AbstractRetryTaskHandler {
	...
	public void execute(RetryTask task, HandlerStatus status) throws Exception {
		...
		repaymentRabbitTemplate.convertAndSend(CommonConfiguration.ROUTING_KEY_PRODUCER_REPAYMENT_SUCCESS, new Gson().fromJson(task.getData(), RepaymentSuccessMessage.class));
