...
public class TzAccountRepaymentSuccessSmsMailNotifyHandler extends AbstractRetryTaskHandler{
	...
	private static Logger logger = LoggerFactory.getLogger(TzAccountRepaymentSuccessSmsMailNotifyHandler.class);
	...
	public void execute(RetryTask task, HandlerStatus status) throws Exception {
		logger.info("收到请求【发送账户还款的短信和邮件】，消息为：{}",task.getData());
		...
		if(StringUtils.isBlank(loanInfoBO.getBusinessProductId()) || !StringUtils.equals(loanInfoBO.getBusinessProductId(), Constants.TRAVELZEN_FINANCE_PRODUCT_ID)){
			logger.info(loanInfoBO.getId() + "放款信息，无需提醒还款方式为【账户还款】");
			return;
		}
		...
		try{
			...
			sendSmsMessage(generateMessageContent(SmsMessageTemplate.tz_account_repayment_success_sms_message_content_template, smsMessageParamList), customerDetailDTO.getContactTel());
		...
		try{
			...
			sendMailMessage(customerDetailDTO.getContactMail(), generateMessageContent(MailMessageTemplate.tz_account_repayment_success_mail_message_content_template, mailMessageParamList), MailMessageTemplate.tz_repayment_success_mail_message_subject_template);
		} catch(WebException e){
			logger.error(e.getMessage(), e);
		} catch(Exception e){
			logger.error("未知异常", e);
