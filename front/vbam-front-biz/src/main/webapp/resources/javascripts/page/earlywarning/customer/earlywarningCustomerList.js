var urlFormat = basepath + "/earlywarning/event/tip/{0}/{1}/{2}" ;
...
$(function(){
    ...
    $("#monitorList-grid").kendoTooltip({
        filter: "a.item-link",
        content: {
        	url: basepath + "/earlywarning/event/tip" 
        },
        width: 650,
        position: "bottom",
        requestStart: function(e) {
            e.options.url = kendo.format(urlFormat, $(e.target.parent().siblings()[0]).text(), e.target.attr('outCustomerId'), e.target.attr('level'));
        }
    });
    
    $('.form-search-out').click(function() {
    	window.open(basepath + "/earlywarning/customer/export/earlywarning?companyName=" + $.trim($("[name='companyName']").val()) + "&earlyWarningLevel=" + $.trim($("[name='earlyWarningLevel']").val()));
    });
	...
	$("#monitorList-grid").kendoGrid({
		...
		dataSource : {
			...
			transport : {
				...
				url: basepath + "/earlywarning/customer/query/earlyWarningCustomer"
			}
		},
		serverPaging: true,
		serverSorting: true,
