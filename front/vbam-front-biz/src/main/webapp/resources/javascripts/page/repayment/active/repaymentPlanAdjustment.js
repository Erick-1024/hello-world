...
$(function(){
  ...
  //关闭修改新费用弹窗
	...
	$("body").delegate("#repaymentConfirm", "click", function() {
		$.ajax({
			type: "POST",
			url: basepath+"/repayment/active/submitValidate",
			data: { redisKey: $.trim($("input[name='redisKey']").val())
			},
			dataType: "json",
			success: function(data){
...
function paidRepaymentPlan(){
    ...
    $("#manualGrid-plan").kendoGrid({
        ...
        columns: [{
            ...
            template: function(data){
            	var url="";
            	if( data.periodStatus != "2" && data.settleStatusForPage=="未结清"){
    ...
    $("#manualGrid-cost").kendoGrid({
        ...
        columns: [{
            ...
            template: function(data){
            	var url="";
            	if(data.settleStatusForPage=="未结清"){
            		url += "<a class='comRow-link editRepayed-link' href='javascript:void(0);' data-id='"+data.id+"'>调整</a>";
            	}
            	return url;
            },
            editable: "popup"
