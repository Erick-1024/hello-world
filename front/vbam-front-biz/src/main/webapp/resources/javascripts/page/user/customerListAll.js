	...
	var grid = $(".companyAcutGrid").kendoGrid({
			...
			transport: {
				read: {
					...
					data: {
							...
							accountAuditStatus: ($.trim($("#accountAuditStatus .active").attr("name")) == "ALL")?"":$.trim($("#accountAuditStatus .active").attr("name")),
		...
		columns: [{
            ...
            template: function(data){
        		...
        		if(reviewAuth && data.accountAuditStatus == "PENDINGAUDIT"){
