...
$(function(){
	...
	$("body").on("click",".relaunchingWithdrawOperate",function(){
		var tradeRecordId = $(this).data("trade");
		$.ajax({
			type: "POST",
			url: basepath + "/account/trade/relaunchingWithdrawOperate",
			data: {tradeRecordId : tradeRecordId},
			dataType: "json",
			success: function(data){
				if(data.status == "SUCCESS"){
					showSuccessWin("提交成功，请耐心等待！");
					searchTradeRecord();
				}else{
...
function searchTradeRecord(){
	...
	var grid = $(".accountManageGrid").kendoGrid({
			...
			transport: {
				read: {
					...
					data: {
							...
							oppositeAccountNo: $.trim($("#oppositeAccountNo").val()).parseBankAccountNo(),
		...
		columns: [{
            ...
            template: function (data) {
            	var returnStr = "";
            	if("WITHDRAW_FUND" == data.tradeType &&  "TRADE_FAIL" == data.status && relaunchingWithdrawAuth){
            		returnStr += "<a class='comRow-link relaunchingWithdrawOperate' href='javascript:void(0);' data-trade=" + data.id + ">重新提现</a>";
            	}
            	return returnStr;
            }
        },{
