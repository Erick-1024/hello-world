...
$(function(){
    ...
    $("body").on("click", ".permissions-link", function(e){
    	var id = $(this).parent().next().text();
		window.open(basepath + "/customer/gotoEditPermissions?userId=" + id);
    });
	...
	var grid = $(".companyAcutGrid").kendoGrid({
        ...
        columns: [{
            ...
            template: function(data){
            	...
            	if(permissionsAuth){
            		url += "<a class='comRow-link permissions-link' href='javascript:void(0);'>个性权限</a>";
            	}
            	return url;
