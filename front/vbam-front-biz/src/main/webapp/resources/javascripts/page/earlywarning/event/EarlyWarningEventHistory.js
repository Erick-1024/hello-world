...
function generateWindow(isPredict) {
	...
	$('.relieve-table tr:eq(2)').after(generateEventDetailContent($('[earlywarningtype]').attr('earlywarningtype'), $('[aria-selected=true] > *:last > a').attr('subType'), $('[aria-selected=true] [name=occurTime]').text(), $('[aria-selected=true] span[name=amount]').text(), $('[earlywarningtype]').attr('earlywarningtype') == 'SYSTEM' ? JSON.parse($('[aria-selected=true] span[represent]').attr('represent')) : $('[aria-selected=true] span[represent]').text(), JSON.parse($('[aria-selected=true] > *:last > a').attr('extra'))));
	...
	if(earlywarningTypeText.attr('earlywarningType') != 'SYSTEM' && earlywarningTypeText.attr('type') =='history') {
	            ...
	            transport: {
	            	read : {
						...
						data: {
							...
							entryReviewTimeEnd: $('[name=entryReviewTimeEnd]').val(),
							occurTimeStart: $('[name=occurTimeStart]').val(),
							occurTimeEnd: $('[name=occurTimeEnd]').val(),
							earlywarningType: $('[earlywarningType]').attr('earlywarningType'),
							isCancel: $('[name=isCancel]').val() 
						},
					url: basepath + "/earlywarning/event/query/earlyWarningEventHistory"
	            }
	        },
	        serverPaging: true,
			serverSorting: true,
	    ...
	    $("#monitorSrl-grid").kendoGrid({
			...
			columns: [{
                ...
                template:function(data){
                	var txt = data.represent.represent + data.represent.standard;
                	var l =txt.length;
                	var outTxt = "";
                	if(l>=50){
