...
function initEvent() {
	...
	$("body").on("click", ".add-cost-btn", function() {
		...
		if($loanState =="LOANED"){
			$(".open-message-btn").click();
			$("#tip-box-window .dlg-notice").addClass("notice-icon01");
			$("#tip-box-window .notice-content").text("已放款不能进行新增");
			return;
		}
	...
	$("body").on("click", ".ch-appoint-btn", function() {
		...
		if ($index != undefined) {
			var $loanInfo = $("#appoint-tb tr").eq($index).find("[name='loanInfoId']").text();
			if($loanInfo != undefined && $loanInfo !=''){
				$(".open-message-btn").click();
				$("#tip-box-window .dlg-notice").addClass("notice-icon01");
				$("#tip-box-window .notice-content").text("已放款不能进行修改");
				return;
			}
			$(".open-appoint-btn").click();
			moneyNum();
	...
	$("body").on("click", ".del-appoint-btn", function() {
		...
		removeClientRow("appoint-tb","INVOICE");
	});
	
	// 删除费用信息
	$("body").on("click", ".del-cost-btn", function() {
		
		removeClientRow("cost-tb","FEE");
	});
}
 ...
 * 表格操作（删除）
 */
function removeClientRow(ckb,type) {
	//获取选中的复选框，然后循环遍历删除
	var ckbs = $("input[name=" + ckb + "]:checked");
	if (ckbs.size() == 0) {
		$(".open-message-btn").click();
    ...
    $("#delete-confirm").click(function(){
    	var $loanInfo = $("#appoint-tb tr").eq(ckbs.parent().parent().index()).find("[name='loanInfoId']").text();
    	var $loanState = $("#loanState").val();
    	if(type == "INVOICE" && $loanInfo !=''){
    		$(".open-message-btn").click();
			$("#tip-box-window .dlg-notice").addClass("notice-icon01");
			$("#tip-box-window .notice-content").text("已放款不能进行删除");
			return;
    	}
		if(type == "FEE" && $loanState =="LOANED"){
			$(".open-message-btn").click();
			$("#tip-box-window .dlg-notice").addClass("notice-icon01");
			$("#tip-box-window .notice-content").text("已放款不能进行删除");
			return;
...
function opAppoint() {
	...
	html = $('<tr class="client-add-tr">' + '<th><input type="checkbox" class="input-new" name="appoint-tb"></th>'
			...
			+ $timeTwo + '</td> ' + '<td style="display:none" name="id"></td>'+'<td style="display:none" name="counterpartyId">' + $("#invCustId").val() +'</td>' + '</tr>');
...
function initInvModle() {
    ...
    var grid = $("#client-out-grid").kendoGrid({
        ...
        dataSource:{
            ...
            transport: {
                read: {
            		data: { customerName: $("#customerName").val(), businessContractNo:$("#businessContractNo").val()},
            		type : "POST",
