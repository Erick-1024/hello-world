...
var columns = [{
                ...
                {
                	var link = "";
                	if(employeeDetailsAuth)
                		link += "<a target='_blank' href='gotoEmployeeRoleDetails?roleId=" + data.id + "'>详情</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
                	if(employeeEditAuth)
                		link += "<a target='_blank' href='gotoEditEmployeeRole?roleId=" + data.id + "'>编辑</a>";
                	return link;
                }
	            }
			];
	...
	dataSource = new kendo.data.DataSource({
					...
					transport: {
						    read: {
						    		  data: serverData,
									  type: "post",
								      url: basepath + "/role/getEmployeeRoleList"	      
								  }
						  		},
					serverPaging: true,
					serverSorting: true,
...
{
	setDataSource();
	$(".staffRoleGrid").remove();
	$("#companyRoleGrid").append("<div class=\"staffRoleGrid\"></div>");
	sequence = 0;
	var kendoGrid = $(".staffRoleGrid").kendoGrid(
					{
