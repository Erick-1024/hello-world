...
function initSearchBtn(){
	...
	//忘记密码
//	forgetPayPwdPopWindow = new PopWindow({
//		title: "忘记支付密码",
//        width: 460,
//        reload: true,
//        resizable: false,
...
//	$("body").on("click", "#forgetPayPwd", function(e){
//		forgetPayPwdPopWindow.open().center();
//	});
	
	//人工操作支付结果提示框
	payTipPopWindow = new PopWindow({
		title: "提示",
		width: 420,
		reload: true,
    ...
    $('body').on('click','.confirm-link',function(){
    	if(parseFloat($(".fromAccountBalance").text().parseMoney())< parseFloat($(".toFee").text().parseMoney())){
//    		payTipPopWindow.open().center();
//			$("#tip-box-window .dlg-notice").addClass("notice-icon01");
//			$("#tip-box-window .notice-content").text("余额不足！");
    		$(".popup-header").html("提示");
    	    $(".pop-con").html("余额不足！");
    	　	//判断数据已确定显示的图标
		...
		setValidator($('#payPassword'),"required",true,payPasswordRule.required);
		...
		$.ajax({
			type:"POST",async:false,dataType:"json",data:{id:transferId,password:$('#payPassword').val()},url:basepath+"/credit/transfer/manual",
			success:function(response){
				if(response.status=='SUCCESS'){
					 $(_parent).find("td:eq(6)").text(response.data);
					 if(response.data!='失败'){
						 $(_parent).find("td:eq(8)").find('a').remove();
					 }
					 closePopWindow();
					 payTipPopWindow.open().center();
					 $("#tip-box-window .notice-content").text("支付"+response.data+"！");
					 if(response.data!='失败')
					...
					payTipPopWindow.open().center();
					$("#tip-box-window .dlg-notice").addClass("notice-icon01");
					$("#tip-box-window .notice-content").text(response.message);
				}
			}
		});
