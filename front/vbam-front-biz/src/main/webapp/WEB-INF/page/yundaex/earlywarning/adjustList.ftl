        ...
        <div class="new-war-table">
            <table>
                ...
                <#list YundaexEarlywarningEventCategory?keys as key>
					<tr>
	                    <td>${YundaexEarlywarningEventCategory[key]}</td>
	                    <#if typeListResponse.earlyWaringEventTypeDTOs?? && typeListResponse.earlyWaringEventTypeDTOs?size != 0>
	                    	...
	                    	<#list typeListResponse.earlyWaringEventTypeDTOs as earlyWaringEventTypeDTO>
	                    		<#if earlyWaringEventTypeDTO.earlywaringEventType == key>
	                    			...
	                    			<td><a class='new-link' href='${basepath}/yundaex/earlywarning/earlyWarningEventHistory?earlywarningEventCategory=${key}&memberId=${typeListResponse.memberId}&outCustomerId=${typeListResponse.outCustomerId}&type=cancel&thirdItem=information'>解除事件</a><a class='new-link' href='${basepath}/yundaex/earlywarning/earlyWarningEventHistory?earlywarningEventCategory=${key}&memberId=${typeListResponse.memberId}&outCustomerId=${typeListResponse.outCustomerId}&type=history&thirdItem=information'>预警事件历史</a></td>
	                    			<#assign isShow=true />
	                    			<#break>
	                    		</#if>
	                    	</#list>
	                    	<#if !isShow>
	                    		<td>0</td>
	                    		<td>-</td>
	                    		<td><a class='new-link' href='${basepath}/yundaex/earlywarning/earlyWarningEventHistory?earlywarningEventCategory=${key}&memberId=${typeListResponse.memberId}&outCustomerId=${typeListResponse.outCustomerId}&type=history&thirdItem=information'>预警事件历史</a></td>
                    		</#if>
	                    <#else>
	                    	<td>0</td>
	                    	<td>-</td>
