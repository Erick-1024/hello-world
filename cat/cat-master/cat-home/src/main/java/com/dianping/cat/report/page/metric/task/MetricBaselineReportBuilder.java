...
public class MetricBaselineReportBuilder implements TaskBuilder, LogEnabled {
	...
	protected void buildDailyReportInternal(Map<String, MetricReport> reports, String reportName, String metricId,
		...
		for (MetricType type : MetricType.values()) {
			...
			List<Double> weights = baselineConfig.getWeights();  // 1.0, 2.0, 3.0, 4.0
			// baselineConfig.getTargetDate() = 7
			Date targetDate = new Date(reportPeriod.getTime() + baselineConfig.getTargetDate() * TimeHelper.ONE_DAY);
			List<double[]> values = new ArrayList<double[]>();
			System.out.println("Metric-Data start : " + new Date());
			for (Integer day : days) {  // days =  -21, -14, -7, 0
			...
			System.out.println("Metric-Data end : " + new Date());
			System.out.println(" values.size : " + values.size());
			for (double[] d : values) {
				System.out.println("d :  " + d.length);
			}
			// weights = 1.0, 2.0, 3.0, 4.0  POINT_NUMBER = 60 * 24
