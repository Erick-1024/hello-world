...
public class ExceptionAlert implements Task {
	...
	protected TopReport queryTopReport(Date start) {
		...
		if (m_topService.isEligable(request)) {
			ModelResponse<TopReport> response = m_topService.invoke(request);		// BaseCompositeModelService
	...
	public void run() {
		...
		while (active) {
			...
			try {
				TopMetric topMetric = buildTopMetric(new Date(current - TimeHelper.ONE_MINUTE * 2));		//查询所有异常
