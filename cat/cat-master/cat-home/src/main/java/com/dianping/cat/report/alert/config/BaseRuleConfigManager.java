...
public abstract class BaseRuleConfigManager {
	...
	private int calMaxNum(Set<Integer> nums) {
       ...
       /**
         * @param config 监控规则配置
         * @return true 如果验证当前时间在监控规则配置的时间段内
         * */
	private boolean checkTimeValidate(Config config) {
        ...
        /**
          * 将监控开始时间和结束时间，当前时间转换成int型，即00:00开始已过分钟数，进行比较
          * @param start 监控开始时间
          * @param end 监控结束时间
          * @return true 如果当前时间大于等于开始时间且小于等于结束时间
          * */
	 ...
	 * @param configs
	 * @return
	 * 把所有开始、结束时间满足的condition组在一起，并把其中最大持续时间作为key
	 */
