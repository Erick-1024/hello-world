 ...
 */
package com.travelzen.framework.mail;
...
public class MailServerImpl implements MailServer {
	...
	private void setUpCCandBCC(Mailable email, Message message) throws MessagingException, AddressException {
	...
	private void addRecipient(Message message, String emails, Message.RecipientType rType) throws MessagingException, AddressException {
	...
	public void send(Mailable email, ArrayList<HashMap<String, String>> file_list) throws MessagingException {
		Session mailSession = Session.getInstance(this.mailConfig, null);
		Message message = new MimeMessage(mailSession);
		...
		// Part two is the attachment
		for (HashMap<String, String> file_map : file_list) {

			messageBodyPart = new MimeBodyPart();
			DataSource source = new FileDataSource(file_map.get("file_path").toString());
			messageBodyPart.setDataHandler(new DataHandler(source));
			messageBodyPart.setFileName(file_map.get("file_name").toString());
			multipart.addBodyPart(messageBodyPart);
		}
	...
	public void sendHTML(Mailable email) throws MessagingException {
		...
		bp.setContent(email.getMessage(), "text/html;charset=utf-8");
	...
	Map<String,Long> lastMessageMap = new HashMap<String,Long>();
	
	@Override
	public void sendBuffered(Mailable email) throws MessagingException {
		
		String key = email.getSubject()+email.getMessage();
		if(!lastMessageMap.containsKey(key)){
			lastMessageMap.put(key, System.currentTimeMillis());
		}else{
			if(System.currentTimeMillis() -lastMessageMap.get(key) < 10*60*1000 ){
				return ;
			}
		}
