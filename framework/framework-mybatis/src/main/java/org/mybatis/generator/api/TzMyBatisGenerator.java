 ...
 * @author Jeff Butler
 */
public class TzMyBatisGenerator {
	 ...
	 */
	public TzMyBatisGenerator(TzConfiguration configuration, ShellCallback shellCallback, List<String> warnings) throws InvalidConfigurationException {
		super();
	...
	public void generate(ProgressCallback callback, Set<String> contextIds, Set<String> fullyQualifiedTableNames) throws SQLException, IOException, InterruptedException {
		...
		List<TzContext> contextsToRun;
		if (contextIds == null || contextIds.size() == 0) {
			contextsToRun = configuration.getContexts();
		} else {
			contextsToRun = new ArrayList<TzContext>();
			for (TzContext context : configuration.getContexts()) {
		...
		if (configuration.getClassPathEntries().size() > 0) {
			...
			TzObjectFactory.addExternalClassLoader(classLoader);
		}

		// now run the introspections...
		int totalSteps = 0;
		for (TzContext context : contextsToRun) {
