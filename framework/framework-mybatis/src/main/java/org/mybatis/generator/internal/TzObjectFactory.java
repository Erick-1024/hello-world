...
import org.mybatis.generator.api.TzJavaFormatter;
import org.mybatis.generator.api.TzXmlFormatter;
...
import org.mybatis.generator.codegen.mybatis3.TzIntrospectedTableMyBatis3Impl;
...
public class TzObjectFactory {
    ...
    public static synchronized void addResourceClassLoader(
        ...
        TzObjectFactory.resourceClassLoaders.add(classLoader);
    }
    ...
    public static synchronized void addExternalClassLoader(
            ClassLoader classLoader) {
        TzObjectFactory.externalClassLoaders.add(classLoader);
    }
    ...
    public static CommentGenerator createCommentGenerator(TzContext context) {
    ...
    public static TzJavaFormatter createJavaFormatter(TzContext context) {
        String type = context.getProperty(PropertyRegistry.CONTEXT_JAVA_FORMATTER);
        if (!StringUtility.stringHasValue(type)) {
            type = DefaultJavaFormatter.class.getName();
        }

        TzJavaFormatter answer = (TzJavaFormatter) createInternalObject(type);
    ...
    public static TzXmlFormatter createXmlFormatter(TzContext context) {
        String type = context.getProperty(PropertyRegistry.CONTEXT_XML_FORMATTER);
        if (!StringUtility.stringHasValue(type)) {
            type = DefaultXmlFormatter.class.getName();
        }

        TzXmlFormatter answer = (TzXmlFormatter) createInternalObject(type);
     ...
     * @return
     */
    public static IntrospectedTable createIntrospectedTableForValidation(TzContext context) {
        String type = context.getTargetRuntime();
        if (!stringHasValue(type)) {
            type = TzIntrospectedTableMyBatis3Impl.class.getName();
