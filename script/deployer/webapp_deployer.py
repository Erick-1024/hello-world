...
class WebappDeployer(DeployBase):
    ...
    def writeLog(self):
        user = getDeployerInfo.getUserName()
        hostName = getDeployerInfo.getHostName()
        ip=''
        try:
            ip = getDeployerInfo.get_ip_address('eth0')
        except:
            pass
        ISOTIMEFORMAT = '%Y-%m-%d %X'
        cmd = 'echo '+'time:%s,deployer:%s,host:%s,ip:%s' %(time.strftime(ISOTIMEFORMAT, time.localtime()),user,hostName,ip)+' > /data/log/%(appname)s.txt' %self.config
        self.sshCmd(cmd)
